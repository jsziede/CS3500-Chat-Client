<?php
	...
	while (true)
	{
		...
		{	
			...
			while(socket_recv($changed_socket, $buf, 1024, 0) >= 1)
			{
				...
				{
					$avatar = $tst_msg->avatar;
					$error=updateAvatarDB($avatar,$uid);
					if($error)
						sendErr("Updating Avatar DB Error");
					//send to all clients
					$arr = array('type'=>'avatar', 'uid'=>$uid,'avatar'=>$avatar);
					$response_text = mask(json_encode($arr));
					send_message($response_text); 				//send data
				}
				else if($type =="color")					//if updating the color
				{
					$color = $tst_msg->colorName;
					$error = updateColorDB($color,$uid);
					if($error)
						sendErr("Updating Color DB Error");
					//send to all clients
					$arr = array('type'=>'color', 'uid'=>$uid,'color'=>$color);
					$response_text = mask(json_encode($arr));
					send_message($response_text); 				//send data
				}
				...
				{
					$error = setTheme($uid,$tst_msg->theme);
					if($error)
						sendErr("Updating theme DB error");
				}
				else if($type =="usermsg")									//if sending a message
				{
					...
					$name = getName($uid);
					$color = getColor($uid);
					$arr = array('type'=>'usermsg', 'uid'=>$uid, 'message'=>$user_message,'time'=>$time,'color'=>$color,'name'=>$name,'avatar'=>$avatar);
					$response_text = mask(json_encode($arr));
					send_message($response_text); 				//send data
				}
	...
	function setTheme($uid,$theme)
	{
		global $dbConnection;
		$sql = "UPDATE prefs SET theme = '".$theme."' WHERE uid = '".$uid."'";
		printf($sql."\n");
	...
	function getAvatar($uid)
	{
		...
		$sql = "SELECT avatar FROM prefs WHERE uid = '".$uid."'";
		printf($sql."\n");
		$result = mysqli_query($dbConnection, $sql);
		if($error = mysqli_error($dbConnection)=="")
		{
	...
	function getColor($uid)
	{
		global $dbConnection;
		//updates the preferred background of the user's name
		$sql = "SELECT color FROM prefs WHERE uid = '".$uid."'";
		printf($sql."\n");
		$result = mysqli_query($dbConnection, $sql);
		if($error = mysqli_error($dbConnection)=="")
		{
	...
	function updateBGDB($uid, $bgName)
	{
		global $dbConnection;
		//updates the preferred background of the user's name
		$sql = "UPDATE prefs SET bg = '".$bgName."' WHERE uid = '".$uid."'";
		printf($sql."\n");
		$result = mysqli_query($dbConnection, $sql);
		if($error = mysqli_error($dbConnection)=="")
			return false;
