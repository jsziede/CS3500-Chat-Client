<?php
	...
	function getHeader() {
	global $dbConnection;
	$sql = "SELECT avatar, uid, color, bg FROM prefs WHERE uid = '".$_SESSION["id"]."'";
	$result = mysqli_query($dbConnection, $sql);
	if(!$result)
	{
		echo "Not logged in.";
	...
	{
		$row = mysqli_fetch_assoc($result);
		echo "<div id='uid' style='display: none'>".$row['uid']."</div><div id='bgid' style='display: none'>".$row['bg']."</div><img id='header-avatar-".$row['uid']."' src='images/avatars/".$row['avatar'].".png' /><p>Welcome, <span id='header-name-".$row['uid']."' style='color: ".$row['color']."'>".$_SESSION['name']."</span>.</p><form id='logout' method='post' action='index.php'><button name='logout' type='submit'>Log out</button></form>";
		return true;
	}
}
...
function showBG() {
	global $dbConnection;
	$sql = "SELECT bg FROM prefs, backgrounds WHERE uid = '".$_SESSION["id"]."'";
	$result = mysqli_query($dbConnection, $sql);
	if(!$result)
	{
	...
	{
		$row = mysqli_fetch_assoc($result);
		echo "<div id='blur' style='background-image: url(\"images/backgrounds/".$row["bg"].".jpg\");'></div>";
		return true;
	}
}
...
function getBgs() {
	...
	else
	{
		while ($row = $result -> fetch_assoc()) {
			$bg = $row["filename"];
			echo "<div style='background: url(\"images/backgrounds/".$bg.".jpg\"); background-size: cover; background-position: center;' class='bg' id='".$bg."'  onclick='changeBG(\"".$bg."\", \"".$_SESSION["id"]."\")'></div>";
		}
		return true;
	}
}
